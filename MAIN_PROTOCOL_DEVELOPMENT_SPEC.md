...
- Status: V1 fixed-price Main Protocol core contracts are implemented. A temporary EOA-admin deployment to Base Sepolia (84532) completed and passed post-deployment verification on 2026-08-18, but it predates restoration of the source-defined five-parameter `DatasetRegistered` event. The current source therefore requires a fresh deployment and post-deployment verification; the earlier addresses are historical test evidence, not a deployment of the current bytecode. Production Safe execution and external audit remain pending.
...
| `ProtocolTimelock` | Implemented, delay/role/execution-tested, and passing | 11 |
| Artifact/deployment/network/Merkle/Manifest/Challenge/deferred-scope assertions | Passing | 40 |
| **Total** | **Full regression passing** | **156** |

The deployment and post-deployment verification scripts share the same importable, integration-tested implementation and are TypeScript-checked. Persistent-network identity and rejection branches are directly unit-tested without broadcasting a deployment. An official Safe v1.5.0 Singleton/proxy integration test requires two owner signatures, checks nonce replay rejection, executes all six onboarding/wiring transactions, and then runs full deployment verification; focused tests also reject unexpected singleton/code, modules, guard, fallback handler, code hashes, and extra privileged role members. Hardhat compilation, formatting, Solidity lint, 98.61% line coverage, 98.57% statement coverage, gas reporting, dependency gates, and the 156-test regression suite pass. Slither 0.11.5 completed on the source preceding this event-only ABI restoration with no high-severity finding and must be rerun for the current release candidate; reviewed non-high findings are recorded in `security/SLITHER_REVIEW.md`. Within the confirmed V1 decisions and explicitly deferred scope, the current source has no known mismatch with this development specification. The historical Base Sepolia EOA deployment is not a production release and no longer represents the current DatasetRegistry bytecode; fresh deployment, production Safe execution, public challenge-intake/Gateway operations, and an independent smart-contract audit remain release gates.
...
| Governance delay and authority lock | Production config changes and UUPS upgrades use the non-upgradeable `ProtocolTimelock`, an OpenZeppelin `TimelockController` with a fixed 48-hour minimum delay. Governance may increase the delay but `updateDelay` cannot reduce it below the enforced minimum. The operational multisig is proposer, executor, and canceller; the Timelock is permanently self-administered and is the core contracts' sole, fixed `DEFAULT_ADMIN_ROLE` holder. Neither the Timelock nor a core contract can grant that role to another address or revoke/renounce it from the Timelock. Configuration setters and UUPS authorization compare the caller directly with the stored Timelock address, so transferring a role cannot create a delay bypass. Emergency pause and challenge operations remain immediate multisig actions through `ADMIN_ROLE`. An explicit short-delay test mode may use a 60-second minimum only for the temporary Base Sepolia EOA deployment and is not a production configuration. |
...
6. Set `challengeStatus = None` and `weightsInvalidated = false`, then emit the source-defined five-parameter `DatasetRegistered`. Manifest URI, digest, and version remain discoverable through the Registry getters.
...
V1 additionally requires events for every listing and challenge state transition so indexers do not need to infer mutable state. Manifest URI, digest, and version are read from the immutable DatasetRegistry getters after receiving `DatasetRegistered`:
...
The four source-defined Main Protocol event signatures are preserved exactly, including the five-parameter `DatasetRegistered` topic. Manifest commitments are exposed through the Registry getters rather than an additional event; the challenge decision adds `WeightChallengePending`. All later events are explicit V1 additions needed to expose the resolved listing, challenge, fee/configuration, operator-assignment, and one-time-wiring state transitions. `RoleGranted`, `RoleRevoked`, `Paused`, `Unpaused`, and UUPS upgrade events use their standard OpenZeppelin definitions.
...
| Required Main Protocol events | Part 4 | The four source event signatures are preserved exactly. Manifest commitments are independently discoverable through Registry getters; additive events cover challenge, listing, settlement, configuration, operator-assignment, and wiring transitions. |
...
| --- | --- |
| Registration | CONTRIBUTOR self-registration; assigned OPERATOR registration; unassigned/unauthorized rejection; every validation failure; immutable registration fields; deadline snapshot; exact source `DatasetRegistered` topic and Manifest getter commitments. |
